...
export const RoleAuthModal: React.FC<RoleAuthModalProps> = ({
  ...
  const handleAdminVerify = (e?: React.FormEvent) => {
    ...
    if (pinInput.trim() === currentSavedPin || pinInput.trim() === '4706') {
      ...
      setPinError('관리자 비밀번호(PIN)가 일치하지 않습니다. (기본: 4706)');
      pinInputRef.current?.focus();
    }
  };
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-indigo-700 dark:text-indigo-300 mt-0.5">
                      관리자 전용 PIN 비밀번호를 입력해 주세요. <span className="font-semibold text-indigo-900 dark:text-indigo-200">(기본 PIN: 4706)</span>
                    </p>
                  </div>
                </div>
                <button
                  ...
                  className="px-2 py-1 rounded-lg text-2xs font-bold bg-indigo-100 dark:bg-indigo-900/60 hover:bg-indigo-200 dark:hover:bg-indigo-800 text-indigo-700 dark:text-indigo-300 transition-all cursor-pointer whitespace-nowrap"
                  title="기본 PIN 번호 4706 자동 입력"
                >
                  기본 4706 입력
                </button>
              </div>
