...
export type ViewTab = 'monthly' | 'daily' | 'students' | 'analytics' | 'kiosk';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="h-16 flex items-center justify-between gap-4">
          ...
          <nav className="flex items-center gap-1.5 overflow-x-auto py-1 scrollbar-none bg-slate-100/80 dark:bg-slate-800/80 p-1 rounded-xl border border-slate-200/80 dark:border-slate-700/80">
            ...
            {/* 2. 일별 빠른 체크: 관리자만 접근 가능 (담임 교사, 학생 비공개) */}
            {userRole === 'admin' && (
              <button
            ...
            {/* 5. 입실 키오스크: 관리자, 학생 모드에서 사용 가능 (담임 교사 모드에서는 조회 전용이므로 숨김) */}
            {userRole !== 'teacher' && (
              <button
                onClick={() => setActiveTab('kiosk')}
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-xs sm:text-sm font-bold transition-all whitespace-nowrap shrink-0 ${
                  activeTab === 'kiosk'
                    ? 'bg-indigo-600 text-white shadow-md shadow-indigo-200 dark:shadow-indigo-950'
                    : 'text-indigo-700 dark:text-indigo-300 bg-indigo-50/60 dark:bg-indigo-950/40 hover:bg-indigo-100 dark:hover:bg-indigo-900/60'
                }`}
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {onSync && (
              <button
                ...
                onClick={onSync}
                disabled={isSyncing}
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 text-xs font-bold transition-all border border-slate-200 dark:border-slate-700 shadow-2xs cursor-pointer disabled:opacity-60"
            ...
            {/* Share / Role Links Modal Button */}
            {onOpenShareModal && (
              <button
                type="button"
                onClick={onOpenShareModal}
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-100 dark:hover:bg-indigo-900/80 text-indigo-700 dark:text-indigo-300 text-xs font-bold transition-all border border-indigo-200 dark:border-indigo-800/80 shadow-2xs cursor-pointer"
                title="관리자/담임교사/학생 역할별 전용 링크 확인 및 복사"
              >
                <Share2 className="w-3.5 h-3.5" />
                <span className="hidden sm:inline">역할별 링크</span>
              </button>
            )}
