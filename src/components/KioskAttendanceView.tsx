...
export const KioskAttendanceView: React.FC<KioskAttendanceViewProps> = ({
  ...
  const eligibleStudents = useMemo(() => {
    return students.filter(st => st.active && !isStudentExcluded(st, session, currentDate, currentDayName));
  }, [students, session, currentDate, currentDayName]);
  ...
  // Allow manual status change from inside the confirmation card if needed
  const handleModalStatusChange = (newStatus: AttendanceStatus) => {
    if (!checkInResult) return;
    const isLate = newStatus === 'LATE';
    onUpdateRecord(checkInResult.student.id, currentDate, newStatus, checkInResult.reason, checkInResult.timeStr);
  ...
  // Save custom reason in modal
  const handleSaveModalReason = () => {
    if (!checkInResult) return;
    onUpdateRecord(checkInResult.student.id, currentDate, checkInResult.status, modalReasonText, checkInResult.timeStr);
    setCheckInResult(prev => prev ? { ...prev, reason: modalReasonText } : null);
    setIsEditingReasonInModal(false);
  };
        ...
        {checkInResult && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/70 backdrop-blur-xs">
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full p-6 sm:p-7 shadow-2xl border-2 border-indigo-500 dark:border-indigo-400 relative overflow-hidden"
            >
              ...
              <div className="space-y-4 pt-1">
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs sm:text-sm font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-50/80 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 px-3 py-1 rounded-xl">
                  ...
                  {(() => {
                    ...
                    return (
                      <span className={`text-xs sm:text-sm font-bold px-3 py-1 rounded-full border flex items-center gap-1 ${curMeta.badgeClass}`}>
                        <StatusIcon status={checkInResult.status} size="xs" />
                ...
                <div className="flex items-center justify-between py-1">
                  <div className="flex items-baseline gap-3">
                    ...
                    <span className="text-xs font-mono font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/50 px-2.5 py-1 rounded-lg">
                      🕒 {checkInResult.timeStr}
                    </span>
                  </div>

                  {checkInResult.student.parentPhone && (
                    <span className="text-xs sm:text-sm font-mono font-bold text-indigo-700 dark:text-indigo-300 bg-indigo-50/80 dark:bg-indigo-950/60 border border-indigo-200 dark:border-indigo-800 px-3 py-1 rounded-xl">
                ...
                {(() => {
                  const rawAcademyDays = getStudentAcademyDays(checkInResult.student);
                  ...
                  return (
                    <div className="my-1 px-3 py-2 rounded-xl bg-slate-50 dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700 flex items-center justify-between gap-2 shadow-2xs">
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {hasAcademyDays ? (
                          <span className="text-xs sm:text-sm font-black text-amber-800 dark:text-amber-200 bg-amber-100 dark:bg-amber-950/70 border border-amber-300/90 dark:border-amber-700/80 px-2.5 py-0.5 rounded-lg tracking-wide">
                            {sortedAcademyDays.map(d => (d.endsWith('요일') ? d : `${d}요일`)).join(', ')}
                          </span>
                        ) : (
                          <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 bg-slate-200/70 dark:bg-slate-700/70 px-2 py-0.5 rounded-md">
                            없음 (매일 참여)
                          </span>
                        )}
                      </div>
                      ...
                      {isTodayAcademyDay && (
                        <span className="shrink-0 text-3xs sm:text-[11px] font-black text-rose-700 dark:text-rose-300 bg-rose-100 dark:bg-rose-950/70 border border-rose-200 dark:border-rose-800 px-2 py-0.5 rounded-md">
                          오늘 학원일 (야자 미참여)
                        </span>
                      )}
                    </div>
                ...
                {(() => {
                  ...
                  return (
                    <div className="space-y-1.5 pt-2 border-t border-slate-100 dark:border-slate-800">
                      {isCurrentlyLate && (
                        <div className="text-3xs text-amber-600 dark:text-amber-400 font-bold px-1 flex items-center gap-1">
                      ...
                      <div className={`grid ${visibleStatuses.length === 4 ? 'grid-cols-4' : 'grid-cols-5'} gap-2`}>
                        {visibleStatuses.map(st => {
                          ...
                          return (
                            <button
                              ...
                              className={`py-2.5 px-1 rounded-2xl text-xs font-bold transition-all flex flex-col items-center justify-center border cursor-pointer ${
                                isSelected
                                  ? `${m.badgeClass} ring-2 ring-indigo-500 scale-[1.04] shadow-md`
                                  : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800'
                              }`}
                            >
                              <span className="flex items-center justify-center text-base leading-none font-bold h-5">
                  ...
                  {isEditingReasonInModal ? (
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        placeholder="사유 입력 (예: 병결, 수행평가, 학원 등)"
                        className="flex-1 px-3 py-1.5 text-xs rounded-xl border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white focus:outline-hidden"
                        autoFocus
                      />
                      <button
                        type="button"
                        onClick={handleSaveModalReason}
                        className="px-3 py-1.5 text-xs font-bold rounded-xl bg-indigo-600 text-white hover:bg-indigo-700"
                      >
                        저장
                      </button>
                      <button
                        type="button"
                        onClick={() => setIsEditingReasonInModal(false)}
                        className="px-2.5 py-1.5 text-xs rounded-xl border border-slate-300 text-slate-500 hover:bg-slate-100"
                      >
                        취소
                      </button>
                    </div>
                ...
                <div className="pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3">
                  <div className="text-2xs text-slate-400 font-medium">
                    {countdown}초 후 자동으로 닫힙니다 (또는 Enter)
                  </div>
