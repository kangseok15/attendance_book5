...
import { WEEKDAYS, getStudentAcademyDays, sortStudents } from '../utils/attendanceHelpers';
import { 
  Users, 
  UserPlus, 
  Edit3, 
        ...
        <table className="w-full text-xs text-left border-collapse">
          <thead>
            <tr className="bg-slate-100 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 font-bold text-slate-700 dark:text-slate-300">
              ...
              <th className="py-3 px-3 text-center min-w-44 whitespace-nowrap">학원 가는 요일 (체크 시 미참여/음영)</th>
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-700/60">
            {filteredStudents.map((st, idx) => {
              ...
              if (isEditing) {
                const editAcademyDays = getStudentAcademyDays(editingStudent);
                        ...
                        {WEEKDAYS.map(dayName => {
                          ...
                          return (
                            <button
                              ...
                              title={`${dayName}요일: ${isAcademy ? '학원 (야자 미참여 음영)' : '학원 없음 (야자 참여 빈칸)'}`}
                            >
                              {dayName}
                            </button>
                          );
              ...
              const stAcademyDays = getStudentAcademyDays(st);
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              ...
                              const allWeekdays = ['월', '화', '수', '목', '금'];
                              const nextNightDays = allWeekdays.filter(d => !nextAcademyDays.includes(d));
                              const updated = students.map(s => (s.id === st.id ? { 
                                ...s, 
                                academyDays: nextAcademyDays,
                                nightDays: nextNightDays 
                              } : s));
                            ...
                            title={`${st.name}: ${dayName}요일 ${isAcademy ? '학원 (야자 미참여, 출석부에 음영 처리)' : '학원 없음 (정상 야자 참여, 출석부에 빈칸)'}`}
                          >
                            {dayName}
                          </button>
                        );
