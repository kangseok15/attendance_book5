...
export const MonthlyGridView: React.FC<MonthlyGridViewProps> = ({
  ...
  const todayDateStr = `${year}-${String(month).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
  const defaultStatDay = activeDays.find(d => d.dateStr === todayDateStr) || activeDays[0];
          ...
          <div className="bg-slate-900 text-white rounded-2xl p-3.5 border border-slate-700 shadow-sm flex flex-col justify-between gap-2.5 shrink-0 self-stretch sm:self-auto sm:min-w-[330px]">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="flex items-center justify-between px-2.5 py-1.5 rounded-xl bg-slate-800/90 border border-slate-700/60">
                ...
                <span className="font-extrabold text-indigo-300 font-mono text-sm">: {sessionAttendanceStats.g1}명</span>
              </div>
              <div className="flex items-center justify-between px-2.5 py-1.5 rounded-xl bg-slate-800/90 border border-slate-700/60">
                <span className="text-slate-300 font-medium">2학년 출석인원</span>
                <span className="font-extrabold text-emerald-300 font-mono text-sm">: {sessionAttendanceStats.g2}명</span>
              </div>
              <div className="flex items-center justify-between px-2.5 py-1.5 rounded-xl bg-slate-800/90 border border-slate-700/60">
                <span className="text-slate-300 font-medium">3학년 출석인원</span>
                <span className="font-extrabold text-purple-300 font-mono text-sm">: {sessionAttendanceStats.g3}명</span>
              </div>
              <div className="flex items-center justify-between px-2.5 py-1.5 rounded-xl bg-indigo-600 text-white border border-indigo-500 font-bold shadow-xs">
                ...
                <span className="font-black text-white font-mono text-sm">: {sessionAttendanceStats.total}명</span>
              </div>
            </div>
          </div>
        )}
                          ...
                          if (isPostNov17) {
                            return (
                              <td
                                ...
                                title={`${student.name} - 11월 17일 이후 3학년 자습 제외`}
                                style={{
